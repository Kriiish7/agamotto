...
/**
 * Demo Convex `users` id for schedule API calls.
 *
 * Stub auth (`src/lib/auth.tsx`) stores a random UUID that is **not** a Convex
 * `Id<"users">`. Until Convex Auth lands, the schedule screen keeps a separate
 * localStorage key for the real Convex user id (same key as the tasks screen).
 *
